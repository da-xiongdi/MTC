...
def keq(T, in_dict):
    ...
    for key, value in in_dict["keq"].items():
        ...
        if key!= '1':
            react_eq_constant[key] = np.exp((value[0] + value[1] * T + value[2] * T ** 2 + value[3] * T ** 3 +
                                         value[4] * T ** 4 + value[5] * T ** 5 + value[6] * T * np.log(T)) / R / T)
...
def rate(T, P, F_dict, feed_data, chem_dict):
    ...
    react_rate = dict()
    driving = rate_const["1"] * ad_const["CO2"] * (Pi["CO2"] * Pi["H2"] ** 1.5 - Pi["Methanol"] * Pi["H2O"] / Pi[
        "H2"] ** 1.5 / eq_const['1'])
    inhibiting = (1 + ad_const["CO"] * Pi["CO"] + ad_const["CO2"] * Pi["CO2"]) * (
            Pi["H2"] ** 0.5 + ad_const["H2O/H2"] * Pi["H2O"])
    react_rate['1'] = driving / inhibiting

    driving = rate_const["2"] * ad_const["CO2"] * (Pi["CO2"] * Pi["H2"] - Pi["CO"] * Pi["H2O"] / eq_const['2'])
    inhibiting = (1 + ad_const["CO"] * Pi["CO"] + ad_const["CO2"] * Pi["CO2"]) * (
            Pi["H2"] ** 0.5 + ad_const["H2O/H2"] * Pi["H2O"])
    ...
    inhibiting = (1 + ad_const["CO"] * Pi["CO"] + ad_const["CO2"] * Pi["CO2"]) * (
            Pi["H2"] ** 0.5 + ad_const["H2O/H2"] * Pi["H2O"])
    react_rate['3'] = driving / inhibiting
    # print(react_rate)
    temp = np.zeros((3, 5))
    for i in range(3):
        key = str(i + 1)
