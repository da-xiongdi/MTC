...
class Insulator(Reaction):
    ...
    def flux(self, Th, P, F_dict):
        ...
        gap_min, rmin, rmax = 1e5, 0, Pi_h["Methanol"]/Pi_h["H2O"]
        # print(rmax)
        ...
        Pi_c_cond = self.p_sat(self.Tc, [r_sel / (1 + r_sel), 1 / (1 + r_sel)])
        Pi_c = pd.Series(self.cold_comp(Pi_h.values, Pi_c_cond), index=self.comp_list)
        xi_c = Pi_c / P
        cold_cond = [self.Tc, xi_c["Methanol"], xi_c["H2O"]]
        hot_cond = [Th, xi_h["Methanol"], xi_h["H2O"]]
        ...
        if dev > 0.1: print(dF, r_sel, dev, 'dev too big')
        return dF, dT, dev
    ...
    def simulator2(self):
        ...
        r = (data[0, -1] - data[0, 0]) / data[0, 0]
        dF_react_rwgs = data[-1][-1]
        dF_react_ch3oh = data[0][-1] - dF_react_rwgs
        dF_react_h2o = dF_react_rwgs + dF_react_ch3oh
        sp_ch3oh = 1 - data[2][-1] / dF_react_ch3oh
        sp_h2o = 1 - data[3][-1] / dF_react_h2o
        print(r, sp_ch3oh, sp_h2o)

        n_col = int(len(performance) / 14)
        save_perfomance = pd.DataFrame(np.array(performance).reshape((n_col, 14)))
        save_perfomance.to_excel('result/result5_BU_perfomance_%s_%s_%s_%s_%s_%s.xlsx' %
                           (self.Dt, self.L,self.T0, self.P0, self.Tc, self.v0))
        save_data = pd.DataFrame(data.T, columns=self.comp_list + ['T'])
        save_data.to_excel('result/result5_BU_%s_%s_%s_%s_%s_%s.xlsx' %
                           (self.Dt, self.L,self.T0, self.P0, self.Tc, self.v0))
